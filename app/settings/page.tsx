    ...
    <div className="min-h-screen bg-slate-50">
      <div className="max-w-2xl mx-auto py-8 px-4">
        ...
        <div className="bg-white rounded-lg border border-slate-200 p-6 mt-6">
          <h2 className="text-lg font-semibold text-slate-800 mb-4">Database Connection</h2>
          <p className="text-sm text-slate-500 mb-4">
            Zogotech credentials for loading live program data (enrollment, success rates, demographics, etc.)
          </p>

          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">
                Zogotech Database Password
              </label>
              <div className="relative">
                <input
                  type={showZogoPassword ? 'text' : 'password'}
                  value={zogoDbPassword}
                  onChange={(e) => setZogoDbPassword(e.target.value)}
                  placeholder="Enter password..."
                  className="w-full px-3 py-2 pr-10 border border-slate-300 rounded-md text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                />
                <button
                  type="button"
                  onClick={() => setShowZogoPassword(!showZogoPassword)}
                  className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600"
                >
                  {showZogoPassword ? (
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <button
              ...
              disabled={!zogoDbPassword || savingDb}
              className="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              {savingDb ? 'Saving...' : 'Save Password'}
            </button>
