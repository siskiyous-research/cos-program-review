...
export function SuccessRateChart({ data, label, showLabels = false }: { data: SuccessRecord[]; label: string; showLabels?: boolean }) {
    ...
    <ResponsiveContainer width="100%" height={280}>
      <LineChart data={data}>
        ...
        <Line type="monotone" dataKey="successRate" stroke="#10b981" name="Success %" strokeWidth={2} dot={{ r: 3 }}
          label={showLabels ? { position: 'top', fontSize: 9, formatter: (v: number) => `${v.toFixed(0)}%` } : false} />
        <Line type="monotone" dataKey="completionRate" stroke="#f59e0b" name="Completion %" strokeWidth={2} dot={{ r: 3 }}
          label={showLabels ? { position: 'bottom', fontSize: 9, formatter: (v: number) => `${v.toFixed(0)}%` } : false} />
      </LineChart>
    </ResponsiveContainer>
  );
}
