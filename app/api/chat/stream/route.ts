 ...
 * Streams chat responses with program context + RAG
 */

import { NextRequest } from 'next/server';
import { streamChatResponse } from '@/lib/gemini-service';
...
export async function POST(req: NextRequest) {
  ...
  try {
    const { message, chatHistory, programData, knowledgeBaseData, programCategory, aggregatedData, reviewContent } = await req.json();

    if (!message || !programData) {
      return new Response(
        JSON.stringify({ ok: false, error: 'Missing required fields: message, programData' }),
    ...
    const { stream, citations } = await streamChatResponse(
      message,
